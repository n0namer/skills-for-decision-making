...
export function selectDecisionMethods(spec) {
  ...
  // No numeric method is a valid result. Decision framing remains a skill step,
  // not a fake calculator/method step called "framing-only".
  return methods;
}
...
export function explainMethodSelection(spec) {
  ...
  if (methods.length === 0) {
    return [{
      method: 'framing-only',
      reason: 'insufficient structured numeric inputs; continue decision framing without a numeric method',
    }];
  }
  ...
  return methods.map((method) => {
    switch (method) {
      ...
      default:
        return { method, reason: 'selected by the deterministic method router' };
    }
  });
}
