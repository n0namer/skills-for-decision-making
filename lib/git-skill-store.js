...
export class GitSkillStore {
  ...
  materialize(source) {
    ...
    const repoDir = join(this.cacheRoot, 'repositories', key);

    if (!existsSync(repoDir)) {
      mkdirSync(join(this.cacheRoot, 'repositories'), { recursive: true });
      ...
      try {
        ...
        // Rename through git-independent filesystem state only after clone validation.
        // Node rename is intentionally avoided here so partial Windows cache cleanup stays simple.
        this.git(['clone', '--no-checkout', '--', source.repository, repoDir]);
