...
const RULES = [
  ...
  },
  {
    ...
    when: (s) => Boolean(s.resourceAllocation),
...
export function inferSignals(text = '') {
  ...
  const signals = {
    ...
    resourceAllocation: has(
      ...
      /allocate/,
      /resource allocation/,
      /\b(split|divide)\b.*\b(time|budget|hours?)\b/,
      /\b(hours?|time|budget)\b.*\b(project|projects|options|channels|campaigns)\b/,
      /where .*put .*time/,
    ),
    portfolioDecision: has(
      /портфел/,
      /несколько проект/,
      /между .*проект/,
      /оцен.*проект/,
      /приорит.*проект/,
      /ранжир.*проект/,
      /какие .*проект.*(делать|остав|пауз|закры)/,
      /portfolio/,
      /(?:priorit|rank|evaluate|compare|choose).*projects?/,
      /active projects?/,
    ),
...
export function routeSkills(signals = {}, registry = []) {
  // Routing answers "what is required?". Availability is validated later by
  // materializePipelineExecution, which fails closed if a required skill is missing.
  // Do not silently drop required work just because the current registry is incomplete.
  void registry;
  const routed = [];
