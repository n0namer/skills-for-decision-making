...
import { assertDecisionSpec } from './decision-spec.js';
...
function buildOrchestration({
  ...
  const effectiveDecisionSpec = decisionSpec ? assertDecisionSpec(decisionSpec) : null;
  ...
  const methods = effectiveDecisionSpec ? selectDecisionMethods(effectiveDecisionSpec) : [];
  const pipeline = planPipeline({ routedSkills, methods });

  return {
    registry,
    decisionSpec: effectiveDecisionSpec,
    result: {
      request,
...
export function prepareOrchestrationExecution(options = {}) {
  ...
  return {
    ...
    execution: materializePipelineExecution({
      ...
      decisionSpec: built.decisionSpec,
    }),
  };
}
